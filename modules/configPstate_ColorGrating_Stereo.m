...
Pstate.param{end+1} = {'min_ori1'         'int'        0       0                'deg'};
Pdoc.param{end+1}='minimum orientation';
Pstate.param{end+1} = {'orirange1'         'int'        360       0                'deg'};
Pdoc.param{end+1}='orientation range';
Pstate.param{end+1} = {'n_ori1'    'int'   16       0                ''};
Pdoc.param{end+1}='number of orientations';


Pstate.param{end+1} = {'min_ori2'         'int'        0       0                'deg'};
Pdoc.param{end+1}='minimum orientation';
Pstate.param{end+1} = {'orirange2'         'int'        360       0                'deg'};
Pdoc.param{end+1}='orientation range';
Pstate.param{end+1} = {'n_ori2'    'int'   16       0                ''};
Pdoc.param{end+1}='number of orientations';

Pstate.param{end+1} = {'KeepSame_ori'   'int'   0     0                ''};
Pdoc.param{end+1}='0=vary both eyes independently 1=same value for both eyes ';

Pstate.param{end+1} = {'min_sf1'   'float'   0.05       0                'c/deg'};
Pdoc.param{end+1}='minimum spatial frequency';
Pstate.param{end+1} = {'max_sf1'   'float'   1       0                'c/deg'};
Pdoc.param{end+1}='maximum spatial frequency';
Pstate.param{end+1} = {'n_sfreq1' 'int'   10       0             ''};
Pdoc.param{end+1}='number of spatial frequencies';
Pstate.param{end+1} = {'sf_domain1'   'string'   'log'       0                ''};
Pdoc.param{end+1}='spatial frequency spacing: log or lin';

Pstate.param{end+1} = {'min_sf2'   'float'   0.05       0                'c/deg'};
Pdoc.param{end+1}='minimum spatial frequency';
Pstate.param{end+1} = {'max_sf2'   'float'   1       0                'c/deg'};
Pdoc.param{end+1}='maximum spatial frequency';
Pstate.param{end+1} = {'n_sfreq2' 'int'   10       0             ''};
Pdoc.param{end+1}='number of spatial frequencies';
Pstate.param{end+1} = {'sf_domain2'   'string'   'log'       0                ''};
Pdoc.param{end+1}='spatial frequency spacing: log or lin';

Pstate.param{end+1} = {'KeepSame_sf'   'int'   0     0                ''};
Pdoc.param{end+1}='0=vary both eyes independently 1=same value for both eyes ';



Pstate.param{end+1} = {'min_tp1'   'int'   1       0                'frames'};
Pdoc.param{end+1}='minimum temporal period';
Pstate.param{end+1} = {'max_tp1'   'int'   50       0                'frames'};
Pdoc.param{end+1}='maximum temporal period';
Pstate.param{end+1} = {'n_tp1' 'int'   10       0             ''};
Pdoc.param{end+1}='number of temporal periods';
Pstate.param{end+1} = {'tp_domain1'   'string'   'log'       0                ''};
Pdoc.param{end+1}='temporal period spacing: log or lin';

Pstate.param{end+1} = {'min_tp2'   'int'   1       0                'frames'};
Pdoc.param{end+1}='minimum temporal period';
Pstate.param{end+1} = {'max_tp2'   'int'   50       0                'frames'};
Pdoc.param{end+1}='maximum temporal period';
Pstate.param{end+1} = {'n_tp2' 'int'   10       0             ''};
Pdoc.param{end+1}='number of temporal periods';
Pstate.param{end+1} = {'tp_domain2'   'string'   'log'       0                ''};
Pdoc.param{end+1}='temporal period spacing: log or lin';

Pstate.param{end+1} = {'KeepSame_tp'   'int'   0     0                ''};
Pdoc.param{end+1}='0=vary both eyes independently 1=same value for both eyes ';

Pstate.param{end+1} = {'n_phase'   'int'   4       0                ''};
Pdoc.param{end+1}='number of phases';

Pstate.param{end+1} = {'n_phase2'   'int'   4       0                ''};
Pdoc.param{end+1}='number of phases';

Pstate.param{end+1} = {'KeepSame_phase'   'int'   0     0                ''};
Pdoc.param{end+1}='0=vary both eyes independently 1=same value for both eyes ';
